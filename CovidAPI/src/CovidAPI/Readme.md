# Challenge 2 - CovidAPI

This is my solution for the CovidAPI challenge of the AWS Skills Uplift Initiative.

Challenge: _In a Covid-Safe office environment, we need to track the movements of staff around the office. Imagine QR codes are installed on every desk, in meeting rooms, and in lunch rooms. The QR Code has the form of a base URL, followed by a Guid that is a unique identifier for that location (e.g. https://iumvsihpv9.execute-api.ap-southeast-2.amazonaws.com/Prod/LocationId= 69ec46cb-6e87-41af-a020-c693fa9553b9), meeting room or lunch room. Users are expected to use their phones to be redirected to a website that calls an API to register that they were there. The API records the users name and phone number and the time they registered. The API can then be queried by date, or by location guid in order to see where people had been._

_Build the API to support this use case using AWS dotnet tooling, API Gateway and Lambda._

## First Pass

The First pass is putting in place an ASP.Net WebAPI using the template `serverless.image.AspNetCoreWebAPI` which uses a container based lambda to produce an API using .Net 5.
The First Pass can be accessed at the Challenge2/FirstPass branch.
...
* covidapi.yml - an AWS CloudFormation Serverless Application Model template file for declaring your Serverless functions and other AWS resources
...
Execute unit tests
```
    cd "CovidAPI/test/CovidAPI.Tests"
    dotnet test
```

Deploy application
```
    cd "CovidAPI/src/CovidAPI"
